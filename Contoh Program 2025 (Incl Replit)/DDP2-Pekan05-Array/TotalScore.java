  ...
  public static void main(String args[]) {
    ...
    for (int i = 0; i < scores.length; i++) {
      ...
      System.out.println("Student " + i + "'s score is " +
        totalScore);
      if (tertinggi < totalScore){
        tertinggi = totalScore;
        indexTertinggi = i;
      }
      if (terendah > totalScore){
        terendah = totalScore;
      }

      totalSeluruh += totalScore;
    }
    System.out.printf("Total Score %.1f dengan rata-rata %.1f \n", 
                      totalSeluruh,
                      totalSeluruh/ scores.length); 

    System.out.println("Nilai tertinggi: " + tertinggi);
    System.out.printf("Dipegang oleh Student %s \n" ,nama[indexTertinggi]);
